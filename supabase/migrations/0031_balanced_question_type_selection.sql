-- Pinoy Quiz — 0031: balanced question-type selection in start_game
--
-- Bug: in Mixed Mode, non-multiple_choice types almost never showed up
-- in a game even though they were enabled. Root cause was in
-- start_game (0030): it filtered the question pool down to the
-- enabled types, then did a single `order by random() limit
-- question_count` over that *entire* pool. Because the pool is
-- overwhelmingly multiple_choice (hundreds of MC questions vs. a
-- handful of seeded rows per new type), a flat random draw almost
-- always lands on multiple_choice by sheer weight of numbers — the
-- other types were reachable but never statistically likely to be
-- picked.
--
-- Fix: replace the single pooled draw with a per-type allocation.
-- question_count is split as evenly as possible across the resolved
-- enabled types (resolve_enabled_question_types, unchanged), any type
-- that doesn't have enough matching rows for its share gives up the
-- shortfall, which is redistributed round-robin to types that still
-- have spare rows (bounded by the same total-availability check as
-- before, so it always resolves). Each type's slots are then filled
-- with `order by random()` *within that type only*, and the final
-- list is shuffled again so same-type questions don't cluster back to
-- back in the game's running order.
--
-- Everything else about start_game (per-type shuffle_map / unscramble
-- letters / match_shuffle / sequence_shuffle generation, the
-- game_questions insert, final game status update) is unchanged from
-- 0030 — only how v_question_ids gets built is different.
...
begin
  ...
  -- Fill each type's slots with a random draw scoped to that type only.
  for v_idx in 1..v_n_types loop
    if v_target[v_idx] > 0 then
      ...
      v_question_ids := v_question_ids || coalesce(v_picked, array[]::uuid[]);
    end if;
  end loop;
