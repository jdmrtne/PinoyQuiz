-- Pinoy Quiz — 0033: fix smallint[]/integer[] type mismatch in the
-- Sequence shuffle retry loop
--
-- Bug report: Start Game failed with "Something went wrong. Please
-- try again." (a 404 from PostgREST) whenever Mixed Mode had enough
-- types enabled that a `sequence` question was likely to be drawn.
-- The underlying Postgres error was:
--
--   42883: operator does not exist: smallint[] <> integer[]
--
-- game_questions.sequence_shuffle is smallint[], so v_seq_shuffle was
-- declared smallint[] too. But generate_series() returns plain
-- integer, so `generate_series(...) x` produces an integer[] once
-- aggregated — and Postgres has no built-in smallint[] <> integer[]
-- operator, so the retry-loop's exit condition below threw 42883 (a
-- Postgres "undefined operator" error, which PostgREST maps straight
-- to HTTP 404 — matching exactly what showed up in the browser).
--
-- This bug shipped with Sequence questions in 0030 and has been dormant
-- since: the original flat random() draw across the whole pool rarely
-- picked a `sequence` question at all, so this code path almost never
-- ran. 0031's per-type balancing guarantees every enabled type gets
-- drawn, which is what finally exercised it.
--
-- Fix: cast both sides of the comparison to smallint[] explicitly —
-- generate_series(...)::smallint[] instead of the bare integer
-- output — matching the pattern multiple_choice's shuffle already
-- used (array[0,1,2,3]::smallint[]) a few lines up. Only that one
-- exit condition changes; everything else is identical to 0032.
--
-- Bug report (0032, unchanged below): "some questions are duplicating"
-- — a question with the same wording showed up twice in one game.
--
