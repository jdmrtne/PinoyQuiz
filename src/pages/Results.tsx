 ...
 *  Darkened + given a contrasting stroke below (see crownStyle) since the
 *  original pale silver was nearly invisible against the light theme's
 *  cream background (--color-ink: #faf5ec). */
const CROWN_COLORS = ["#FFC400", "#A6AEBB", "#B5651D"];

/** Shared look for every crown icon: a dark outline + soft drop-shadow so
 *  it reads clearly against either the dark or light theme background,
 *  regardless of how light the fill color itself is. */
function crownStyle(color: string): React.CSSProperties {
  return {
    color,
    fill: color,
    stroke: "#241a08",
